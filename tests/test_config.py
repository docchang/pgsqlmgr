...
class TestLocalHost:
    ...
    def test_local_host_custom_port(self):
        ...
        config = LocalHost(superuser="postgres", port=5433)
        assert config.port == 5433

    def test_local_host_invalid_port(self):
        """Test local host with invalid port."""
        with pytest.raises(ValidationError):
            LocalHost(superuser="postgres", port=70000)
...
class TestConfigLoading:
    ...
    def test_get_host_config(self):
        ...
        try:
            ...
            assert host_config.superuser == "postgres"
        finally:
            config_path.unlink()
    ...
    def test_list_hosts(self):
        ...
        config_data = {
            "hosts": {
                "local": {"type": "local", "superuser": "postgres"},
                "remote": {"type": "ssh", "ssh_config": "remote", "superuser": "postgres"}
            }
        }
