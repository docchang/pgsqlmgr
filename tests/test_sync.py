...
class TestDatabaseSyncManager:
    ...
    def test_init_local_to_local(self):
        ...
        source_config = LocalHost(superuser="postgres")
        dest_config = LocalHost(superuser="postgres", port=5433)
    ...
    def test_init_local_to_ssh(self):
        ...
        source_config = LocalHost(superuser="postgres")
        dest_config = SSHHost(ssh_config="test", superuser="postgres")
    ...
    def test_init_ssh_to_local(self):
        ...
        source_config = SSHHost(ssh_config="test", superuser="postgres")
        dest_config = LocalHost(superuser="postgres")
    ...
    def test_create_local_dump_success(self, mock_mkdtemp, mock_run):
        ...
        source_config = LocalHost(superuser="postgres", password="test123")
        dest_config = LocalHost(superuser="postgres", port=5433)
        sync_manager = DatabaseSyncManager(source_config, dest_config)
    ...
    @patch('subprocess.run')
    def test_create_ssh_dump_failed(self, mock_run):
        """Test SSH database dump creation failure."""
        mock_run.return_value = Mock(returncode=1, stderr="SSH pg_dump failed")
        ...
        assert success is False
        assert "SSH pg_dump failed" in message
    ...
    @patch('subprocess.run')
    def test_transfer_dump_file_ssh_success(self, mock_run):
        """Test dump file transfer with SSH success."""
        mock_run.return_value = Mock(returncode=0)
        ...
        assert success is True
        assert "File transfer handled by source SSH download" in message
    ...
    def test_restore_local_dump_success(self, mock_run):
        ...
        source_config = LocalHost(superuser="postgres")
        dest_config = LocalHost(superuser="postgres", port=5433, password="test123")
        sync_manager = DatabaseSyncManager(source_config, dest_config)
    ...
    @patch('subprocess.run')
    def test_restore_ssh_dump_failed(self, mock_run):
        """Test SSH database restore failure."""
        mock_run.return_value = Mock(returncode=1, stderr="Failed to create database")
    ...
    def test_list_local_databases_success(self, mock_run):
        ...
        host_config = LocalHost(superuser="postgres", password="test123")
    ...
    def test_list_local_databases_failure(self, mock_run):
        ...
        host_config = LocalHost(superuser="postgres")
        sync_manager = DatabaseSyncManager(host_config, host_config)
    ...
    @patch('subprocess.run')
    def test_list_ssh_databases_failed(self, mock_run):
        """Test SSH database listing failure."""
        mock_run.return_value = Mock(returncode=1, stderr="Failed to list databases")

        host_config = SSHHost(ssh_config="test", superuser="postgres")
        sync_manager = DatabaseSyncManager(host_config, host_config)
        ...
        assert success is False
        assert databases == []
        assert "Failed to list databases" in error_msg
